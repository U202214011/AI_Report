...
def build_markdown_constraints_text(selected_dim_titles: List[str]) -> str:
    ...
    if selected_dim_titles:
        ...
        dim_part = (
            "3) 在'维度关键发现'章节下，只能使用以下二级标题（名称必须完全一致）：\n"
            f"{dim_lines}\n"
            "4) 不得输出未在上述列表中的维度二级标题。\n"
            #"5) 若某个允许维度数据不足，可在该标题下说明'数据不足'，但不能改标题、不能删标题。\n"
        )
    else:
        dim_part = (
            "3) 本次未选择维度。可以删除'维度关键发现‘该一级标题。\n"
            "4) 一级标题改为：一、数据概览/二、原因分析/三、综合建议。\n"
        )
    tail = "5) 不要输出任何图片占位符（如 {{image:...}}）,图片由系统后处理插入。"
